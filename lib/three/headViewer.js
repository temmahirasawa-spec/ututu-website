/* Founders のアバタービューア（全身・カラー版）
   ...
   Next.js 版では、このファイル自体はバンドラが内容ハッシュ付きの名前で出すので
   JSに版番号は要らない。**GLBのURLの ?v= は必須**（端末が古いGLBを掴み続ける）。
...
const VER = '20260821a';   // 洋輔さんのGLBを追加
...
/* 2人を並べたときの見え方。
   **枠は2人とも同じ世界の高さを写す。**そのうえで身長ぶんだけ差をつける。
   こうしないと、それぞれが自分の枠いっぱいに収まって同じ背丈に見える。
   身長は枠の data-cm（cm）。いちばん高い人を 1.0 として比を取る。
   足元は共通の基準線（FRAME_H の下端）に揃える */
const FRAME_H = 1.0;    // 写す高さ。いちばん高い人がちょうど収まる
const FRAME_W = 0.62;   // 写す幅。人の立ち姿の目安（腕を広げるぶんは少しはみ出てよい）
...
/* つまずく。k は 0〜1 の強さ。洋輔さんぶん。
   驚き（applyStartle）と同じく「掛けて、描いて、すぐ戻す」ので溜まらない。

   走りは2人とも同じクリップなので、**個性はタップの反応で出す**。
   のけぞる（驚き）に対して、こちらは前へつんのめる。
   顔だけは前を向かせること。うつむかせると転倒に見えて後味が悪い */
function applyStumble(b, k){
  if (!k) return;
  // 上体が前に折れる
  nudge(b.spine,  X,  0.26 * k);
  nudge(b.spine1, X,  0.15 * k);
  nudge(b.spine2, X,  0.10 * k);
  nudge(b.neck,   X,  0.10 * k);
  /* **顔は必ず前を向かせること。**上体の前傾ぶんを首と頭で打ち消す。
     うつむかせると「つまずいた」ではなく「転んだ」に見えて後味が悪い */
  nudge(b.head,   X, -0.46 * k);
  // 腕は前に投げ出して体を支えにいく。左右で高さを変えると崩れが出る
  nudge(b.lShoulder, Z,  0.14 * k); nudge(b.rShoulder, Z, -0.20 * k);
  nudge(b.lArm, X, -1.20 * k);      nudge(b.rArm, X, -1.35 * k);
  nudge(b.lArm, Z,  0.18 * k);      nudge(b.rArm, Z, -0.30 * k);
  nudge(b.lFore, X, -0.50 * k);     nudge(b.rFore, X, -0.28 * k);
  // 右足が突っかかり、左足が流れる
  nudge(b.rUpLeg, X, -0.50 * k);    nudge(b.rLeg, X,  0.80 * k);
  nudge(b.lUpLeg, X,  0.26 * k);    nudge(b.lLeg, X,  0.45 * k);
  nudge(b.rFoot,  X,  0.40 * k);
}

/* ---------- 1体ぶんの組み立て ---------- */
async function makeAvatar(slot, refCm){
  const name = slot.getAttribute('data-head');
  if (!name) return null;
  const cm = +slot.getAttribute('data-cm') || refCm;
  const tap = slot.getAttribute('data-tap') || 'startle';   // 反応の種類
  /* **同じ枠に二重に作らない。**開発中は effect が2回走る。
     印は「待つ前」に付けること。GLBの読み込みを挟むので、
     .live が付くのは最後であり、同時に呼ばれると素通りしてしまう
     （実際に canvas が枠あたり2つできた）*/
  if (slot.dataset.headBusy) return null;
  slot.dataset.headBusy = '1';

  const canvas = document.createElement('canvas');
  let renderer;
  try {
    renderer = new THREE.WebGLRenderer({ canvas, alpha: true, antialias: true });
  } catch { delete slot.dataset.headBusy; return null; }   // WebGL不可 → プレースホルダのまま
  ...
  try {
    ...
    renderer.dispose();
    delete slot.dataset.headBusy;   // 失敗したら印を戻す。残すと二度と作れない
    return null;
  }
  ...
  const bones = {
    ...
    lUpLeg: model.getObjectByName('LeftUpLeg'), rUpLeg: model.getObjectByName('RightUpLeg'),
    lLeg: model.getObjectByName('LeftLeg'),     rLeg: model.getObjectByName('RightLeg'),
    lFoot: model.getObjectByName('LeftFoot'),   rFoot: model.getObjectByName('RightFoot')
  };
  ...
  /* 採寸して、身長の比で縮め、足元を基準線に揃える。
     **model → inner → pivot** の三段。
     inner が「大きさと足元の位置」、pivot が「回転と跳ね」を持つ。
     ひとつにまとめると、跳ねの量まで身長比で変わってしまう */
  const box = poseBounds(model, mixer, clips.length ? clips[0] : null);
  const center = box.getCenter(new THREE.Vector3());
  const size = box.getSize(new THREE.Vector3());
  const rel = Math.max(0.5, Math.min(1, cm / refCm));   // いちばん高い人を1.0とした背丈
  const k = (FRAME_H / size.y) * rel;                   // 高さ1に正規化してから身長比を掛ける
  const pivot = new THREE.Group();
  const inner = new THREE.Group();
  model.position.sub(center);        // 体の中心を原点へ
  inner.add(model);
  inner.scale.setScalar(k);
  // 縮めたあとの半分の高さぶん持ち上げると、足元が枠の下端に乗る
  inner.position.y = -FRAME_H / 2 + (FRAME_H * rel) / 2;
  pivot.add(inner);
  ...
  function frame(){
    ...
    camera.aspect = w / h;
    // 高さと幅のどちらも収まる距離に引く。
    // **2人で同じ値を使うこと。**モデルごとの採寸で決めると、
    // それぞれが枠いっぱいに収まって身長差が消える
    const t = Math.tan(THREE.MathUtils.degToRad(camera.fov / 2));
    // 注視点はずらさない。上げると足元が枠の下端で切れる
    const dH = (FRAME_H / 2) * 1.09 / t;
    const dW = (FRAME_W / 2) * 1.15 / (t * camera.aspect);
  ...
  /* 枠の高さは、まわりの文字や画像が落ち着いてから決まることがある。
     window の resize だけを見ていると、潰れた大きさのまま残る
     （実測で 258x54 のまま固まった）。枠そのものを見張る */
  if (typeof ResizeObserver !== 'undefined'){
    st.ro = new ResizeObserver(function(){ frame(); });
    st.ro.observe(slot);
  }
...
function tick(now){
  ...
  heads.forEach(function(st){
    ...
    if (st.startleT < 1){
      ...
      if (st.tap === 'stumble') applyStumble(st.bones, k); else applyStartle(st.bones, k);
    }

    // 跳ね（タップの驚き）：放物線で浮いて、少しのけぞる
    let hopY = 0, lean = 0;
    if (st.hopT < 1){
      st.hopT = Math.min(1, st.hopT + dt / (st.tap === 'stumble' ? 0.62 : 0.45));
      const w = Math.sin(st.hopT * Math.PI);
      if (st.tap === 'stumble'){
        // つまずき：跳ねずに沈み、前へつんのめってから戻る
        hopY = -w * st.h * 0.045;
        lean =  w * 0.15;
      } else {
        hopY =  w * st.h * 0.035;
    ...
    const breathe = (!REDUCE && !st.mixer) ? Math.sin(now / 900) * st.h * 0.002 : 0;
...
export async function initHeads(slots){
  /* **基準はいちばん背の高い人。**枠ごとに正規化すると身長差が消える */
  const refCm = slots.reduce(function(m, s){
    return Math.max(m, +s.getAttribute('data-cm') || 0);
  }, 0) || 170;
  const made = await Promise.all(slots.map(function(slot){
    return makeAvatar(slot, refCm).catch(function(e){
...
/* 片付け。ページを離れるときや、開発中の作り直しで呼ぶ。
   **canvas を消して .live を外すこと。**残すと次の初期化で二重になる */
export function stopHeads(){
  setRunning(false);
  observers.forEach(function(io){ io.disconnect(); });
  observers.length = 0;
  heads.forEach(function(st){
    try { st.mixer && st.mixer.stopAllAction(); } catch (e) {}
    ...
    st.slot.classList.remove('live');
    delete st.slot.dataset.headBusy;
  });
  heads.length = 0;
}
