...
1ページに統合（本人判断）。見立ては**「お品書き」と「注文伝票」**。
会社の情報はメニューとして読ませ、問い合わせは注文として受ける。
「品書きを見る → 注文する」という店の流れをページの中に再現している。
...
- **確定していない情報はお品書きに足さないこと。**所在地・創業月は登記の
  公開待ちで載せていない
...
- ハンバーガー：PRODUCTS（#after）／ ORDER・REVIEW（外部・新タブ）／
  COMPANY（/company）／ CONTACT（/company#contact）
- 締め（#next）にCTA：`.cta-main`（お問い合わせ）＋ `.cta-sub`（会社概要）。
  **塗りはトークン（--tx/--pg）で書くこと。**直に色を書くと墨反転から取り残される
- Founders の下に「会社概要を見る」（.profile-btn の使い回し）
- フッターの「（準備中）」を実リンクに
...
- お品書きの**所在地・創業月**（登記の公開待ち。確定したら CompanyClient の MENU に足す）
