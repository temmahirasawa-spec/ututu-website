...
- **`-webkit-backdrop-filter` を手で書き足さないこと。**標準と両方書くと
  Lightning CSS が畳んで**プレフィックス付きだけを出力**し、Chrome では効かなくなる
  （メニューの幕と `.linkbtn` が実際にそうなっていた）。標準だけ書けば圧縮側が足す
- **節の見出しは canvas で解ける**（`components/after/pixelReveal.ts`）。
  下の §ピクセルで解ける見出し を参照。**字は DOM に残っている**ので、
  読み上げ・検索・選択には影響しない
...
### ピクセルで解ける見出し（components/after/pixelReveal.ts）

`#after` の節の見出し（`.pxr`）だけ、静かに現れるのではなく
**読み順に1行ずつ、粗い升目から解けて字になる**。先端ほど粗く、
色はアクセント（`--acc`）から本来の文字色（`--tx`）へ寄っていく。

**本文・小見出し・KVのコピーには広げないこと。**節の見出しという役ひとつに
限定しているから語彙として成立している。全部に掛けるとうるさくなる。

- **字は DOM に残す。**canvas は解けきるまでのあいだ上に重ねるだけで、
  終わったら消して本物の字に戻す。読み上げ・検索・選択はそのまま効く
- **行と改行位置は `Range` から実測する。**canvas 側で折り返しを組み直すと
  DOM の折り方と食い違い、終わりの差し替えで字が跳ぶ。実測なら DOM が
  折った通りの位置に同じ字を置ける（実機で重ねて、二重にならないことを確認済み）
- **色はトークンから読む。**計算済みの `color` は紙⇄墨の反転の途中経過を
  拾うので、`#after` の `--tx` / `--acc` を直接見る。これで反転にも追従する
- **戻すときに `color` の遷移を効かせないこと。**`#after h3` には反転用の
  `transition:color .9s` が載っているので、そのまま外すと透明から0.9秒かけて
  滲み出てくる。JS が付け外しの瞬間だけ inline で打ち消している
- 粗さと色は段ごとに**焼いてある**（`LAYERS` 枚）。時間で変わるのは
  「どこを見せるか」だけなので、毎フレーム作り直す必要はない
- 調整するなら先頭の定数（`MAXB` 升の大きさ／`BAND` 帯の長さ／
  `LINE_MS` 1行の時間／`LINE_LAG` 行の重なり）
- `prefers-reduced-motion: reduce` では何もしない。素の見出しがそのまま出る
